...
struct WindowContext
{
  ...
  explicit WindowContext(const sf::Window* w) : window(w), windowHasFocus(window->hasFocus()) {}
...
{
  const bool needReplacement = (s_currWindowCtx->window->getSystemHandle() == window.getSystemHandle());
                 ...
                 const sf::Color&    tintColor)
{
  ...
  ImTextureID textureID = convertGLTextureHandleToImTextureID(texture.getNativeHandle());
  return ImGui::ImageButton(id, textureID, ImVec2(size.x, size.y), uv0, uv1, toImColor(bgColor), toImColor(tintColor));
}
...
void updateJoystickAxis(ImGuiIO&           io,
                        ImGuiKey           key,
...
void updateJoystickAxisPair(ImGuiIO&           io,
                            ImGuiKey           key1,
                            ImGuiKey           key2,
                            sf::Joystick::Axis axis,
                            float              threshold,
